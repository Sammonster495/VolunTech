...
import { groupSubscriptionHandler, incidentSubscriptionHandler, ngoTaskAddedSubscriptionHandler, ngoTaskModifiedSubscriptionHandler, pendingReportSubscriptionHandler, timestampToDate, timestampToTime, userTaskAddedSubscriptionHandler, userTaskModifiedSubscriptionHandler, verifiedReportSubscriptionHandler } from "@/firebase/ngo/tasks";
...
export default function Tasks() {
    ...
    useEffect(() => {
        if(!userloading && user) {
            const unsubscribe = userTaskAddedSubscriptionHandler(user, setUserTasks);
    ...
    useEffect(() => {
        if(!userloading && user) {
            const unsubscribe = userTaskModifiedSubscriptionHandler(setUserTasks);
    ...
    useEffect(() => {
        if(!userloading && user) {
            const unsubscribe = ngoTaskAddedSubscriptionHandler(user, setNgoTasks);
    ...
    useEffect(() => {
        if(!userloading && user) {
            const unsubscribe = ngoTaskModifiedSubscriptionHandler(setNgoTasks);
    ...
    useEffect(() => {
      if(!userloading && user) {
        const unsubscribePending = pendingReportSubscriptionHandler(setPendingReports);

        const unsubscribeVerified = verifiedReportSubscriptionHandler(user, setVerifiedReports);
  ...
  useEffect(() => {
    if(!userloading && user) {
      const unsubscribe = groupSubscriptionHandler(user, setGroups);
